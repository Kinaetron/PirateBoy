...
static void apply_state(Memory* memory, Register* registers, cJSON* state)
{
	registers->af.high = json_u8(state, "a");
	registers->af.low	= json_u8(state, "f");
	registers->bc.high = json_u8(state, "b");
	registers->bc.low	= json_u8(state, "c");
	registers->de.high = json_u8(state, "d");
	registers->de.low  = json_u8(state, "e");
	registers->hl.high = json_u8(state, "h");
	registers->hl.low  = json_u8(state, "l");

	registers->program_counter.value = json_u16(state, "pc");
	registers->stack_pointer = json_u16(state, "sp");
...
static int state_matches(Memory* memory, Register* registers, cJSON* state, char* failure_detail, size_t detail_size)
{
	struct 
	{ 
		const char* name; 
	...
	byte_checks[] =
	{
		{ "a", registers->af.high, json_u8(state, "a") },
		{ "f", registers->af.low,  json_u8(state, "f") },
		{ "b", registers->bc.high, json_u8(state, "b") },
		{ "c", registers->bc.low,  json_u8(state, "c") },
		{ "d", registers->de.high, json_u8(state, "d") },
		{ "e", registers->de.low,  json_u8(state, "e") },
		{ "h", registers->hl.high, json_u8(state, "h") },
		{ "l", registers->hl.low,  json_u8(state, "l") },
	};
	...
	if (registers->program_counter.value != expected_program_counter)
	{
		snprintf(failure_detail, detail_size, "program counter: expected 0x%04X, got 0x%04X",
			expected_program_counter, registers->program_counter.value);

		return 0;
	}

	uint16_t expected_stack_pointer = json_u16(state, "sp");
	if (registers->stack_pointer != expected_stack_pointer)
	{
		snprintf(failure_detail, detail_size, "stack pointer: expected 0x%04X, got 0x%04X",
			expected_stack_pointer, registers->stack_pointer);
...
static void run_opcode_test_file(const char* filename)
{
	...
	cJSON_ArrayForEach(test_case, root)
	{
		...
		registers->af.value = 0;
		registers->bc.value = 0;
		registers->de.value = 0;
		registers->hl.value = 0;

		registers->stack_pointer = 0;
		registers->program_counter.value = 0;

		cpu_reset_state();

		cJSON* initial = cJSON_GetObjectItem(test_case, "initial");
		apply_state(memory, registers, initial);

		cpu_step(memory, registers);

		cJSON* final_state = cJSON_GetObjectItem(test_case, "final");
		char detail[128];

		if (!state_matches(memory, registers, final_state, detail, sizeof(detail)))
		{
			failed++;
